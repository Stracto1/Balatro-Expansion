...
Descriptions.T_Jimbo.Jokers[mod.Jokers.BULL] = " {{ColorChips}}+2{{CR}} Chips for every {{ColorYellow}}1${{CR}} you have#{{ColorGray}} (Currently {{ColorChips}}+[[VALUE]]{{ColorGray}} Chips)"
Descriptions.T_Jimbo.Jokers[mod.Jokers.INVISIBLE_JOKER] = "After {{ColorYellorange}}2{{CR}} rounds, sell this joker to {{ColorYellorange}}Duplicate{{CR}} a random Joker#{{ColorGray}} (Currently [[VALUE]]/2 rounds)"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ABSTRACT_JOKER] = "{{Damage}} {{ColorMult}}+0.15 {{CR}} Damage per Joker held #{{Damage}} {{ColorMult}}+0.03 {{CR}} Damage per Collectible held"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MISPRINT] = "{{Damage}} {{ColorMult}}+0-1 {{CR}} random Damage Up every new room #{{ColorMint}}10% Chance{{CR}} for any item to become a gliched one upon spawning"
Descriptions.T_Jimbo.Jokers[mod.Jokers.JOKER_STENCIL] = "{{ColorMult}}+0.75X{{CR}} Damage Multiplier for every Joker slot being {{ColorYellorange}}empty{{CR}} or containing another {{ColorYellorange}}Joker Stencil{{CR}}#Not having any {{Collectible}}Active item gives and additional {{ColorMult}}+0.25X{{CR}} Damage Multiplier"
Descriptions.T_Jimbo.Jokers[mod.Jokers.STONE_JOKER] = " {{ColorChips}}+1.25 {{CR}}Tears for every for every stone card in the player's full deck #Gives {{ColorChips}}+0.05 {{CR}}Tears per rock in the current room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ICECREAM] = " {{ColorChips}}+4 {{CR}}Tears#Loses {{ColorChips}}-0.16{{CR}} Tears every room completed while held #A trail of slowing follows Jimbo #Killing an enemy on top of the creep turns it in a Frozen Statue"
Descriptions.T_Jimbo.Jokers[mod.Jokers.POPCORN] = "{{Damage}} {{ColorMult}}+1 {{CR}}Damage #Loses {{ColorMult}}-0.2{{CR}} Damage when clearing any blind"
Descriptions.T_Jimbo.Jokers[mod.Jokers.RAMEN] = "{{Damage}} {{ColorMult}}X1.5 {{CR}}Damage Multiplier#loses {{ColorMult}}-0.01X{{CR}} per card discarded while holding this"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ROCKET] = "Gives 2 {{Coin}}Coins on Blind clear#{{Coin}}Coin payout increases by {{ColorYellorange}}1${{CR}} when a boss blind is defeated #Grants the {{Collectible583}}Rocket in Jar effect"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ODDTODD] = " {{ColorChips}}+0.31 {{CR}} Tears for every Odd numbered card triggered this room # {{ColorChips}}+0.07{{CR}} Tears per Collectible held if the total amount is odd"
Descriptions.T_Jimbo.Jokers[mod.Jokers.EVENSTEVEN] = "{{Damage}} {{ColorMult}}+0.04 {{CR}} Damage per Even numbered card triggered this room #{{Damage}} {{ColorMult}}+0.02 {{CR}} Damage per Collectible held if the total amount is even"
Descriptions.T_Jimbo.Jokers[mod.Jokers.HALLUCINATION] = "{{ColorMint}} 1/2 chance {{CR}} to spawn a random {{Card}}{{ColorPink}} Tarot Card {{CR}}when opening a Booster Pack #{{ColorMint}}50% chance{{CR}} to give a {{Collectible431}} Multidimansional Baby every room entered"
Descriptions.T_Jimbo.Jokers[mod.Jokers.GREEN_JOKER] = "{{Damage}}{{ColorMult}}+0.04 {{CR}} Damage per room completed while held#{{Damage}}{{ColorMult}}-0.16 {{CR}}Damage on every hand discard"
Descriptions.T_Jimbo.Jokers[mod.Jokers.RED_CARD] = "{{Damage}}{{ColorMult}}+0.15 {{CR}} Damage when skipping a Booster Pack"
Descriptions.T_Jimbo.Jokers[mod.Jokers.VAGABOND] = "Spawns a random {{Card}}{{ColorPink}} Tarot Card {{CR}}when compleating a room with 2 {{Coin}} Coins or less #{{ColorMint}}1/10 chance{{CR}} to spawn a random {{ColorYellorange}}Beggar{{CR}} when clearing a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.RIFF_RAFF] = "Spawns a random {{ColorChips}} common {{CR}} Joker on every Blind Clear"
Descriptions.T_Jimbo.Jokers[mod.Jokers.GOLDEN_JOKER] = "Gives 4 {{Coin}} Coins on every Blind clear #{{Coin}} Every enemy touched becomes golden for a short time"
Descriptions.T_Jimbo.Jokers[mod.Jokers.FORTUNETELLER] = "{{Damage}}{{ColorMult}}+0.04 {{CR}} Damage for every {{Card}}{{ColorPink}} Tarot Card {{CR}}used throughout the run"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BLUEPRINT] = "Copies the effect of the Joker to its right"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BRAINSTORM] = "Copies the effect of the leftmost held Joker"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MADNESS] = "Destroys another random Joker and gains {{ColorMult}}+0.1X{{CR}} Damage Multiplier every {{ColorYellorange}}Small and Big Blind{{CR}} cleared"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MR_BONES] = "{{PlayerJimbo}}Revives jimbo at Full Hp if he died after clearing at least half of the current Blind or during any {{BossRoom}} Bossfight #{{ColorMint}}20% chance{{CR}} to use the {{Collectible545}} Book of the Dead on every Room Clear"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ONIX_AGATE] = "{{Damage}} {{ColorMult}}+0.07 {{CR}} Damage for every Club card triggered in the room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ARROWHEAD] = " {{ColorChips}}+0.5 {{CR}} Tears per {{ColorGray}}Spade{{CR}} card triggered in the room #  {{ColorChips}}+0.5 {{CR}} Tears per {{Key}} Key held"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BLOODSTONE] = "{{ColorMint}} 1/2 chance {{CR}}for every Heart card triggered to give {{Damage}} {{ColorMult}}X1.05 {{CR}} Damage Multiplier"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ROUGH_GEM] = "{{ColorMint}} 1/2 chance {{CR}}for every Diamond card triggered to spawn a {{Coin}}Coin that disappears shortly after #Every {{ColorYellorange}}Dimaond{{CR}} card shot also has the {{Collectible506}} Backstabber effect"

Descriptions.T_Jimbo.Jokers[mod.Jokers.GROS_MICHAEL] = "{{Damage}} {{ColorMult}}+0.75 {{CR}}Damage #{{Warning}} {{ColorMint}}1/6 chance{{CR}} of getting destoroyed on every Blind cleared"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CAVENDISH] = "{{Damage}} {{ColorMult}}X1.5 {{CR}}Damage multiplier #{{Warning}} {{ColorMint}}1/6 chance{{CR}} of getting destroyed on every Blind cleared"
Descriptions.T_Jimbo.Jokers[mod.Jokers.FLASH_CARD] = "{{Damage}} {{ColorMult}}+0.02 {{CR}} Damage for every {{RestockMachine}} Restock machine activated while holding this #{{Collectible105}} {{Damage}} {{ColorMult}}+0.02 {{CR}} Damage for every {{ColorYellorange}}Dice Item{{CR}} ussed while holding this"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SACRIFICIAL_DAGGER] = "Destroys the Joker to it's right and gains {{Damage}}{{ColorMult}} +0.08 x the destroyed Joker's sell value{{CR}} Damage on every Blind cleared"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CLOUD_NINE] = "Gain 1 {{Coin}} Coin for every {{ColorYellorange}}9{{CR}} in your FullvDeck on every Blind clear #{{Seraphim}} Gives Flight"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SWASHBUCKLER] = "{{Shop}} Gives {{Damage}}{{ColorMult}} +0.05 x the total sell value of other held Jokers{{CR}} Damage"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CARTOMANCER] = "Spawns 1 random {{Card}} {{ColorPink}}Tarot Card{{CR}}on every Blind clear #{{ColorMint}}5% Chance{{CR}} for any pickup to be replaced with a random {{ColorPink}}Tarot Card{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.LOYALTY_CARD] = "{{Damage}} {{ColorMult}}X2 {{CR}} Damage multiplier once every 6 rooms cleares"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SUPERNOVA] = "When a card is played, gives {{Damage}}{{ColorMult}} +0.01 {{CR}} Damage for every time a card with the same value got played in the current room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DELAYED_GRATIFICATION] = "Gives 1 coin per {{Heart}} Heart when clearing a room at full health #{{Timer}} Both the {{Boss}}Bossrush and {{Hush}} Hush door open regardless of the ingame timer"
Descriptions.T_Jimbo.Jokers[mod.Jokers.EGG] = "The sell value of this Joker increases by{{ColorYellorange}} +2$ {{CR}} every {{ColorYellorange}}Blind cleared{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DNA] = "If the {{ColorYellorange}} First {{CR}}card triggered in a {{ColorYellorange}}Blind{{CR}} hits an enemy, a copy of that card is added to the deck #{{Collectible658}} Spawns a Mini-Isaac every Hostile room cleared"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SMEARED_JOKER] = "{{ColorMult}}Hearts{{CR}} and {{ColorYellorange}}Diamonds{{CR}} count as the same suit #{{ColorGray}}Spades{{CR}} and {{ColorBlue}}Clubs{{CR}} count as the same suit"

Descriptions.T_Jimbo.Jokers[mod.Jokers.SLY_JOKER] = "{{ColorChips}}+4 {{CR}}Tears if a {{ColorYellorange}}Pair{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.WILY_JOKER] = "{{ColorChips}}+8 {{CR}}Tears if a {{ColorYellorange}}Three of a kind{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CLEVER_JOKER] = "{{ColorChips}}+7 {{CR}}Tears if a {{ColorYellorange}}Two-pair{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DEVIOUS_JOKER] = "{{ColorChips}}+12 {{CR}}Tears if a {{ColorYellorange}}Straight{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CRAFTY_JOKER] = "{{ColorChips}}+9 {{CR}}Tears if a {{ColorYellorange}}Flush{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.JOLLY_JOKER] = "{{ColorMult}}+1 {{CR}}Damage if a {{ColorYellorange}}Pair{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ZANY_JOKER] = "{{ColorMult}}+2 {{CR}}Damage if a {{ColorYellorange}}Three of a kind{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MAD_JOKER] = "{{ColorMult}}+1.75 {{CR}}Damage if a {{ColorYellorange}}Two-pair{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CRAZY_JOKER] = "{{ColorMult}}+3 {{CR}}Damage if a {{ColorYellorange}}Straight{{CR}} is held when entering a room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DROLL_JOKER] = "{{ColorMult}}+2.25 {{CR}}Damage if a {{ColorYellorange}}Flush{{CR}} is held when entering a room"

Descriptions.T_Jimbo.Jokers[mod.Jokers.MIME] = "{{ColorYellorange}}Retriggers{{CR}} {{ColorYellorange}}all held in hand{{CR}} effects #{{ColorMint}}20% Chance{{CR}} to copy the effect of an Active Item or consumable used"
Descriptions.T_Jimbo.Jokers[mod.Jokers.FOUR_FINGER] = "{{ColorYellorange}}Flushes{{CR}} and {{ColorYellorange}}Straights{{CR}} only need 4 compatible cards #Every fifth non-boss enemy spawned gets killed immediantly"
Descriptions.T_Jimbo.Jokers[mod.Jokers.LUSTY_JOKER] = "{{ColorMult}}+0.03 {{CR}}Damage per triggered {{ColorMult}}Heart{{CR}} card {{ColorYellorange}}triggered{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.GREEDY_JOKER] = "{{ColorMult}}+0.03 {{CR}}Damage per triggered {{ColorYellorange}}Diamond{{CR}} card {{ColorYellorange}}triggered{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.WRATHFUL_JOKER] = "{{ColorMult}}+0.03 {{CR}}Damage per triggered {{ColorGray}}Spade{{CR}} card {{ColorYellorange}}triggered{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.GLUTTONOUS_JOKER] = "{{ColorMult}}+0.03 {{CR}}Damage per triggered {{ColorChips}}Club{{CR}} card {{ColorYellorange}}triggered{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MERRY_ANDY] = "{{ColorRed}}+2 Hp{{CR}} and {{ColorCyan}}-2 Hand size{{CR}} while held"
Descriptions.T_Jimbo.Jokers[mod.Jokers.HALF_JOKER] = "{{ColorMult}}+1.5 {{CR}}Damage when {{CR}}5 or less{{CR}} card have been played in the current room #{{ColorMult}}+1 {{CR}}Damage when holding {{ColorYellorange}}3 or less{{CR}} cards"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CREDIT_CARD] = "Allows to go in {{ColorYellorange}}debt{{CR}} by spending more coins than you have#{{Shop}} Items on sale can be bought with a maximum of {{ColorMult}}-20${{CR}} debt #!!! Interest and coin-based effect cannot trigger while in debt"

Descriptions.T_Jimbo.Jokers[mod.Jokers.BANNER] = "{{ColorChips}}+1.5 {{CR}}Tears per Red heart Jimbo has"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MYSTIC_SUMMIT] = "{{ColorMult}}+0.75 {{CR}}Damage while having only 1 full Red Heart"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MARBLE_JOKER] = "Adds a {{ColorGray}}Stone card{{CR}} to the deck every blind cleared"
Descriptions.T_Jimbo.Jokers[mod.Jokers._8_BALL] = "{{ColorMint}}1/8 Chance{{CR}} to spawn a random {{Card}} {{ColorPink}}Tarot card{{CR}} when an {{ColorYellorange}}8{{CR}} is triggered"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DUSK] = "The {{ColorYellorange}}last 10{{CR}} triggerable cards shot get retriggered an additional time"
Descriptions.T_Jimbo.Jokers[mod.Jokers.RAISED_FIST] = "The held card with the {{ColorYellorange}}lowest value{{CR}} gives {{ColorMult}}0.05{{CR}} {{ColorYellorange}} X its value{{CR}} Damage"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CHAOS_CLOWN] = "Every {{Shop}} Shop visited has an additional Restock Machine"
Descriptions.T_Jimbo.Jokers[mod.Jokers.FIBONACCI] = "{{ColorMult}}+0.08 {{CR}}Damage per {{ColorYellorange}}Ace, 2, 3, 5 and 8{{CR}} triggered this room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.STEEL_JOKER] = "This gains {{ColorMult}}+0.15X {{CR}}Damage Multiplier per {{ColorSilver}}Steel card{{CR}} in your deck"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SCARY_FACE] = "{{ColorChips}}+0.3 {{CR}}Tears per {{ColorYellorange}}Face card{{CR}} triggered #{{ColorChips}}+1 {{CR}}Tears per {{ColorYellorange}}Champion enemy{{CR}} killed this room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.HACK] = "Played {{ColorYellorange}}2, 3, 4 and 5{{CR}} get retrigged #Jimbo gains a copy of every {{Quality0}} and {{Quality1}} Item held"
Descriptions.T_Jimbo.Jokers[mod.Jokers.PAREIDOLIA] = "Every played card counts as a {{ColorYellorange}}Face card{{CR}} #{{ColorMint}}1/5 Chance{{CR}} for any non-boss enemy to become a {{ColorYellorange}}Champion{{CR}}"

Descriptions.T_Jimbo.Jokers[mod.Jokers.SCHOLAR] = "{{ColorMult}}+0.04{{CR}} Damage and {{ColorChips}}+0.1{{CR}} Tears per {{ColorYellorange}}Ace{{CR}} triggered"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BUSINESS_CARD] = "{{ColorMint}}1/2 Chance{{CR}} to spawn 2 vanishing Pennies when a {{ColorYellorange}}Face card{{CR}} is triggered #Gain the {{Collectible602}} Member Card effect"
Descriptions.T_Jimbo.Jokers[mod.Jokers.RIDE_BUS] = "{{ColorMult}}+0.01 {{CR}}Damage per consecutive {{ColorYellorange}}non-Face card{{CR}} triggered"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SPACE_JOKER] = "{{ColorMint}}1/20 Chance{{CR}} to upgrade the played card value's level #Gains an additional {{ColorMint}}1/20 Chance{{CR}} per {{ColorYellorange}}Star-related{{CR}} Item held"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BURGLAR] = "Sets your Hp to {{ColorRed}}1{{CR}}#Cards can be triggered until the deck gets {{ColorYellorange}}reshuffled{{CR}} every room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BLACKBOARD] = "{{ColorMult}}x2 {{CR}}Damage Multiplier if all cards held in hand are {{ColorGray}}Spades{{CR}} or {{ColorChips}}Clubs{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.RUNNER] = "This joker gains {{ColorChips}}+2{{CR}} Tears when entering an {{ColorYellorange}}unexplored{{CR}} room while holding a {{ColorYellorange}}Straight{{CR}} in hand #Gives {{ColorChips}}+Tears{{CR}} equal to your speed stat#{{Speed}} +0.1 Speed"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SPLASH] = "Triggers all cards in hand upon entering a hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BLUE_JOKER] = "{{ColorChips}}+0.1 {{CR}}Tears per remaining card in your deck"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SIXTH_SENSE] = "If the {{ColorYellorange}}First{{CR}} card played in a {{ColorYellorange}}Blind{{CR}} is a {{ColorYellorange}}6{{CR}} and hits an enemy, it gets destroyed and spawns a {{ColorBlue}}Spectral Pack{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CONSTELLATION] = "This Jokers gains {{ColorMult}}+0.04X{{CR}} Damage Multiplier every time a {{ColorCyan}}Planet Card{{CR}} is used while holding it"
Descriptions.T_Jimbo.Jokers[mod.Jokers.HIKER] = "{{ColorYellorange}}Triggering{{CR}} a card gives it a {{ColorYellorange}}permanent{{CR}} {{ColorMult}}+0.02{{CR}} Tears upgrade"
Descriptions.T_Jimbo.Jokers[mod.Jokers.FACELESS] = "Gain {{ColorMult}}+3${{CR}} when discarding {{ColorYellorange}}at least 2 Face cards{{CR}} at the same time #If only {{ColorYellorange}}Non-boss champion{{CR}} enemies are left in the room, they are {{ColorYellorange}}killed instantly{{CR}} spawning a vanishing penny"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SUPERPOSISION] = "Entering an {{ColorYellorange}}unexplored{{CR}} room while holding a {{ColorYellorange}}Straight{{CR}} and an {{ColorYellorange}}Ace{{CR}} spawns an {{ColorPink}}Arcana Pack{{CR}} #Grants 2 {{Collectible128}} Forever Alone Flies"
Descriptions.T_Jimbo.Jokers[mod.Jokers.TO_DO_LIST] = "A random {{ColorYellorange}}Card value{{CR}} is chosen upon entering an hostile room #{{ColorYellorange}}Playing{{CR}} a card with that value spawns a vanishing penny"

Descriptions.T_Jimbo.Jokers[mod.Jokers.CARD_SHARP] = "Gives {{ColorChips}}X1.1{{CR}} Damage Multiplier when {{ColorYellorange}}Scoring{{CR}} 2 cards with the same value consecutevely #{{Collectible665}} Allows Isaac to see the content of chests and sacks"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SQUARE_JOKER] = "Gives {{ColorChips}}+4{{CR}} Tears every fourth card shot #{{Pill}} Gains {{ColorChips}}+0.4{{CR}} Tears every time a {{ColorYellorange}}Retro Vision{{CR}} pill is used"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SEANCE] = "Spawns a {{ColorYellorange}}Spectral Pack{{CR}} if a {{ColorYellorange}}Straight Flush{{CR}} is held when entering a room #{{Collectible727}} {{ColorMint}}1/6 chance{{CR}} to spawn a {{ColorYellorange}}Hungry Soul{{CR}} when killing an enemy"
Descriptions.T_Jimbo.Jokers[mod.Jokers.VAMPIRE] = "When a card with an {{ColorYellorange}}Enhancement{{CR}} is {{ColorYellorange}}Scored{{CR}}, this gains {{ColorChips}}+0.02X{{CR}} Damage Multiplier and removes the card's {{ColorYellorange}}Enhancement{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SHORTCUT] = "Allows {{ColorYellorange}}Straights{{CR}} to be made with gaps of {{ColorYellorange}}1 rank{{CR}} #{{Collectible84}} Creates a {{ColorYellorange}}Crawl space{{CR}} in a random room every floor #{{Collectible84}} immediantly opens all {{ColorYellorange}}Secret Room{{CR}} doors"
Descriptions.T_Jimbo.Jokers[mod.Jokers.HOLOGRAM] = "This gains {{ColorMult}}+0.1X {{CR}}Damage Multiplier when a card gets {{ColorYellorange}}Added{{CR}} to the deck"

Descriptions.T_Jimbo.Jokers[mod.Jokers.BARON] = "Every {{ColorYellorange}}King{{CR}} held in hand gives {{ColorMult}}X1.25{{CR}} Damage Multiplier #{{Quality4}} Gives {{ColorMult}}X1.25{{CR}} Damage Multiplier per Q4 item held"
Descriptions.T_Jimbo.Jokers[mod.Jokers.OBELISK] = "This gains {{ColorMult}}+0.05X{{CR}} Damage Multiplier when {{ColorYellorange}}scoring{{CR}} a card with a different {{ColorYellorange}}Suit{{CR}} from the last"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MIDAS] = "Every {{ColorYellorange}}Face{{CR}} card {{ColorYellorange}}Played{{CR}} becomes {{ColorYellorange}}Golden{{CR}}#{{ColorYellorange}}Champion{{CR}} enemies are breifly turned into golden statues when spawning"
Descriptions.T_Jimbo.Jokers[mod.Jokers.LUCHADOR] = "{{ColorYellorange}}Selling{{CR}} this joker causes a {{Collectible483}}Mama Mega! explosion and weakens all {{ColorYellorange}}Bosses{{CR}} in the room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.PHOTOGRAPH] = "The first {{ColorYellorange}}Face{{CR}} card {{ColorYellorange}}Played{{CR}} every room gives {{ColorMult}}X1.1{{CR}} Damage Multiplier when {{ColorYellorange}}Scored{{CR}} #Gives {{ColorMult}}X1.1{{CR}} Damage Multiplier when killing the first {{ColorYellorange}}Champion{{CR}} enemy in the room "
Descriptions.T_Jimbo.Jokers[mod.Jokers.GIFT_CARD] = "Every joker held gains {{ColorChips}}+1${{CR}} selling value when clearing a blind"
Descriptions.T_Jimbo.Jokers[mod.Jokers.TURTLE_BEAN] = "Gains {{ColorCyan}}+3{{CR}} Hand Size #Bonus is decreased by {{ColorRed}}-1{{CR}} every blind beaten"
Descriptions.T_Jimbo.Jokers[mod.Jokers.EROSION] = "This gains {{ColorChips}}+0.15{{CR}} Damage when a card gets {{ColorYellorange}}Destroyed{{CR}} #This gains {{ColorChips}}+0.1{{CR}} Damage when a {{ColorYellorange}}Tinted Rock{{CR}} gets {{ColorYellorange}}Destroyed{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.RESERVED_PARK] = "When clearing a room every {{ColorYellorange}}Face{{CR}} card held has a {{ColorMint}}1/4 chance{{CR}} to spawn a penny #When killing an enemy every other {{ColorYellorange}}Champion{{CR}} enemy has a {{ColorMint}}1/2 chance{{CR}} to spawn a penny around it"

Descriptions.T_Jimbo.Jokers[mod.Jokers.MAIL_REBATE] = "A random {{ColorYellorange}}Card value{{CR}} is chosen upon entering an hostile room #{{ColorYellorange}}Discarding{{CR}} a card with that value spawns 2 {{Coin}} pennies"
Descriptions.T_Jimbo.Jokers[mod.Jokers.TO_THE_MOON] = "Gain {{ColorYellow}}+1${{CR}} per {{ColorYellorange}}Interest{{CR}} at the end of a blind"
Descriptions.T_Jimbo.Jokers[mod.Jokers.JUGGLER] = "{{ColorChips}}+1 Hand Size{{CR}} #+1 {{ColorYellorange}}consumable slot{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DRUNKARD] = "{{Heart}}{{ColorBlue}} +1 Health up{{CR}} #Healing when at full heath makes Isaac vision more distorted #Taking damage reverts the distorsion"
Descriptions.T_Jimbo.Jokers[mod.Jokers.LUCKY_CAT] = "This gains {{ColorMult}}+0.02X{{CR}} Damage Multiplier when a {{ColorMint}}Lucky Card{{CR}} procs or when a {{Slotmachine}} Slot pays out"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BASEBALL] = "Gives {{ColorMult}}X1.25{{CR}} Damage Multiplier per {{ColorLime}}Uncommon{{CR}} Joker held #Gives {{ColorMult}}X1.1{{CR}} Damage Multiplier per {{Quality2}} Item held"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DIET_COLA] = "Copies the {{Collectible347}} Diplopia effect when {{ColorYellorange}}Sold{{CR}}"

Descriptions.T_Jimbo.Jokers[mod.Jokers.TRADING_CARD] = "{{PlayerJimbo}}{{ColorYellorange}}Destroys{{CR}} every card in hand when {{ColorYellorange}}Sold{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SPARE_TROUSERS] = "{{PlayerJimbo}}This gains {{ColorMult}}+0.25{{CR}} Damage if a {{ColorYellorange}}Two-pair{{CR}} is held when entering a room #Gives {{ColorMult}}+0.1{{CR}} Damage for every active costume the player has"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ANCIENT_JOKER] = "A random {{ColorYellorange}}Card suit{{CR}} is chosen upon entering an hostile room #Gives {{ColorMult}}X1.05{{CR}} Damage Multiplier every time a card with that {{ColorYellorange}}Suit{{CR}} is {{ColorYellorange}}Triggered{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.WALKIE_TALKIE] = "{{ColorChips}}+0.1{{CR}} Tears and {{ColorChips}}+0.04{{CR}} Damage per {{ColorYellorange}}10{{CR}} and {{ColorYellorange}}4{{CR}} triggered this room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SELTZER] = "Every card played in the next 15 rooms gets {{ColorYellorange}}Retriggered{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SMILEY_FACE] = "Gives {{ColorMult}}+0.05{{CR}} Damage per {{ColorYellorange}}Face card{{CR}} triggered this room #Gives {{ColorMult}}+0.15{{CR}} Damage per {{ColorYellorange}}Champion{{CR}} enemy killed this room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CAMPFIRE] = "This gains {{ColorMult}}+0.2X{{CR}} Damage Multiplier per sold Joker this {{ColorYellorange}}Ante{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CASTLE] = "A random {{ColorYellorange}}Card suit{{CR}} is chosen upon entering an hostile room #This gains {{ColorChips}}+0.04{{CR}} Tears when {{ColorYellorange}}Discarding{{CR}} a card with that {{ColorYellorange}}Suit{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.GOLDEN_TICKET] = "Gain {{ColorYellow}}+2${{CR}} every time a {{ColorYellorange}}Golden card{{CR}} is triggered #Every pickup has a {{ColorMint}}1/25 chance{{CR}} to become its {{ColorYellorange}}Golden{{CR}} variant"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ACROBAT] = "Gives {{ColorMult}}X2{{CR}} Damage Multipier if Jimbo has {{ColorYellorange}}5 or less{{CR}} playable cards left"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SOCK_BUSKIN] = "{{ColorYellorange}}Face card{{CR}} played get {{ColorYellorange}}Retriggered{{CR}} #On {{ColorYellorange}}Champion kill{{CR}} Joker effects get retriggered"
Descriptions.T_Jimbo.Jokers[mod.Jokers.TROUBADOR] = "{{ColorChips}}+2 Hand Size{{CR}} #{{ColorBlue}}-5 playable cards{{CR}} every room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.THROWBACK] = "This gains {{ColorMult}}+0.05X{{CR}} Damage Multiplier per skipped {{ColorYellorange}}Special room{{CR}} skipped this run"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CERTIFICATE] = "A card with a {{ColorYellorange}}Random Seal{{CR}} is added to the deck every time a {{ColorYellorange}}Blind{{CR}} is cleared"
Descriptions.T_Jimbo.Jokers[mod.Jokers.HANG_CHAD] = "The {{ColorYellorange}}first{{CR}} card played is {{ColorYellorange}}triggered{{CR}} 3 additional times #The {{ColorYellorange}}first{{CR}} card used every floor spawns a copy of itself"
Descriptions.T_Jimbo.Jokers[mod.Jokers.GLASS_JOKER] = "This gains {{ColorMult}}+0.1X{{CR}} Damage Multiplier every time a {{ColorGlass}}Glass Card{{CR}} is destroyed"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SHOWMAN] = "Alredy seen {{ColorYellorange}}Jokers{{CR}} and {{ColorYellorange}}Items{{CR}} can reappear throughuot the run"
Descriptions.T_Jimbo.Jokers[mod.Jokers.FLOWER_POT] = "{{ColorChips}}X1.5{{CR}} Damage Multiplier if every {{ColorYellorange}}Suit{{CR}} is held in hand when entering an hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.WEE_JOKER] = "This gains {{ColorMult}}+0.04{{CR}} Tears when a {{ColorYellorange}}2{{CR}} is {{ColorYellorange}}triggered{{CR}}#The samller Jimbo is, the higer is the stat gain"
Descriptions.T_Jimbo.Jokers[mod.Jokers.OOPS_6] = "Every {{ColorMint}}Listed Chance{{CR}} is Doubled"
Descriptions.T_Jimbo.Jokers[mod.Jokers.IDOL] = "A random {{ColorYellorange}}Card{{CR}} from Jimbo's deck is chosen upon entering an hostile room #Playing a card with the same {{ColorYellorange}}Value ans Suit{{CR}} gives {{ColorMult}}X1.1{{CR}} Damage Multiplier"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SEE_DOUBLE] = "{{ColorMult}}X1.2{{CR}} Damage Multiplier when entering an hostile room while holding both a {{ColorClub}}Club{{CR}} and a {{ColorYellorange}}Not-club{{CR}} card in hand"
Descriptions.T_Jimbo.Jokers[mod.Jokers.MATADOR] = "Gain {{ColorYellow}}+6${{CR}} when Jimbo takes damage from a {{ColorYellorange}}Boss{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.HIT_ROAD] = "This gains {{ColorMult}}+0.2X{{CR}} Damage Multiplier every time a {{ColorYellorange}}Jack{{CR}} is discarded #!!! Resets every beaten {{ColorYellorange}}Blind{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DUO] = "{{ColorMult}}X1.25{{CR}} Damage Mutiplier if a {{ColorYellorange}}Pair{{CR}} is held when entering an hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.TRIO] = "{{ColorMult}}X1.5{{CR}} Damage Mutiplier if a {{ColorYellorange}}Three of a Kind{{CR}} is held when entering an hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.FAMILY] = "{{ColorMult}}X2{{CR}} Damage Mutiplier if a {{ColorYellorange}}Four of a Kind{{CR}} is held when entering an hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ORDER] = "{{ColorMult}}X2.5{{CR}} Damage Mutiplier if a {{ColorYellorange}}Straight{{CR}} is held when entering an hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.TRIBE] = "{{ColorMult}}X1.35{{CR}} Damage Mutiplier if a {{ColorYellorange}}Flush{{CR}} is held when entering an hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.STUNTMAN] = "{{ColorMult}}+12.5{{CR}} Tears #{{ColorChips}}-3 Hand Size{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SATELLITE] = "Upon Clearing a {{ColorYellorange}}Blind{{CR}} gain {{ColorYellow}}+1${{CR}} per unique {{ColorCyan}}Planet Card{{CR}} used this run"
Descriptions.T_Jimbo.Jokers[mod.Jokers.SHOOT_MOON] = "{{ColorMult}}+0.7{{CR}} Damage per {{ColorYellorange}}Queen{{CR}} held in hand when entering an hostile room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.DRIVER_LICENSE] = "{{ColorMult}}X1.5{{CR}} Damage Multiplier if Jimbo has at least {{ColorYellorange}}18 Enhanced cards{{CR}} in his deck #Having either the {{Adult}} Adult, {{Mom}} Mom or {{Bob}} Bob transformation also activates the effect"
Descriptions.T_Jimbo.Jokers[mod.Jokers.ASTRONOMER] = "Every {{ColorCyan}}Planet card{{CR}},{{ColorCyan}}Celestial Pack{{CR}} and {{ColorYellorange}}Star-themed{{CR}} item is free"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BURNT_JOKER] = "{{ColorCyan}}Upgrades{{CR}} the level of {{ColorYellorange}}discarded{{CR}} cards that hit an enemy"
Descriptions.T_Jimbo.Jokers[mod.Jokers.BOOTSTRAP] = "{{ColorMult}}+0.1{{CR}} Damage per 5 coins held"

Descriptions.T_Jimbo.Jokers[mod.Jokers.CANIO] = "This gains {{ColorMult}}+0.25X{{CR}} Damage Multiplier when a {{ColorYellorange}}Face card{{CR}} is {{ColorYellorange}}Destroyed{{CR}}"
Descriptions.T_Jimbo.Jokers[mod.Jokers.TRIBOULET] = "{{ColorMult}}X1.1{{CR}} Damage Multiplier per {{ColorYellorange}}King or Queen{{CR}} triggered this room"
Descriptions.T_Jimbo.Jokers[mod.Jokers.YORICK] = "This gains {{ColorMult}}+0.2X{{CR}} Damage Multiplier every {{ColorYellorange}}23{{CR}} cards discarded"
Descriptions.T_Jimbo.Jokers[mod.Jokers.CHICOT] = "Every {{ColorYellorange}}Boss>{{CR}} is permanently {{Weakness}} weakened #Disables every curse"
Descriptions.T_Jimbo.Jokers[mod.Jokers.PERKEO] = "Spwns a duplicate of every {{ColorYellorange}}Consumable{{CR}} Jimbo has upon entering a {{Shop}} shop or a new floor"

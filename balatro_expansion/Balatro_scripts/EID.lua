if EID then
    ...
    local function BalatroGoldenConditions(descObj) -- descObj contains all informations about the currently described entity
        if descObj.ObjType == EntityType.ENTITY_PICKUP and descObj.ObjVariant == PickupVariant.PICKUP_TRINKET and descObj.ObjSubType >= TrinketType.TRINKET_GOLDEN_FLAG then 
            ...
            if Trinket >= TrinketType.TRINKET_INVISIBLE_JOKER and Trinket <= TrinketType.TRINKET_LOYALTY_CARD then --CHECKS IF IT'S A MOD'S TRINKET
            ...
            if Config:HasCustomTag("value") then
                if Config:HasCustomTag("mult") then
                    ...
                    if Trinket == TrinketType.TRINKET_SUPERNOVA then
                        if PlayerManager.AnyoneHasTrinket(TrinketType.TRINKET_SUPERNOVA) then
...
------------------LIST OF ALL THE BASIC DESCRIPTIONS--------------------
    ...
    EID:addTrinket(TrinketType.TRINEKT_SWASHBUCKLER, "{{Shop}} This gives {{Damage}} Damage on every shop purchase# {{Damage}}Damage given increases basing on bought item's cost #{{DevilRoom}} Devil deals give specific ammount of {{Damage}} Damage Basing on cost", "Swashbucler", "en_us")
    EID:addTrinket(TrinketType.TRINEKT_SWASHBUCKLER, "{{Shop}} Questo conferisce {{Damage}} Danno ogni Oggetto comprato# il {{Damage}} Danno conferisto aumenta con colsto dell'oggetto comprato #{{DavilRoom}} Devil deals conferiscono {{Damage}} Danno specifico in base al costo", "Moschettiere", "it")
